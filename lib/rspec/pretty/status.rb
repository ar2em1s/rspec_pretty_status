...
require_relative 'status/version'
  ...
  module Pretty
    module Status
      ...
      matcher :have_http_status do |status|
        match do |response|
          raise ArgumentError, 'Invalid http status' unless Rspec::Pretty::Status::STATUSES.key?(status)

          response.status == Rspec::Pretty::Status::STATUSES[status.to_sym]
        end

        failure_message do |response|
          status = Rspec::Pretty::Status::INVERTED_STATUSES[response.status]
